...
interface GeneratedIdQrModalProps {
  treeID: string | null;
  onClose: () => void;
}

export function GeneratedIdQrModal({ treeID, onClose }: GeneratedIdQrModalProps) {
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      await Sharing.shareAsync(uri, {
        mimeType: "image/png",
        dialogTitle: `Share ${treeID} QR Code`,
        UTI: "public.png",
      });
    } catch {
  ...
  return (
    <Modal visible={!!treeID} animationType="fade" transparent onRequestClose={onClose}>
      <View style={styles.backdrop}>
        ...
        >
          ...
          <View ref={captureRefView} collapsable={false} style={styles.captureArea}>
            ...
            <Text style={styles.title}>{treeID}</Text>
            <Text style={styles.subtitle}>Unassigned Tree ID</Text>
          ...
          <Text style={styles.hint}>
            Scan to read this tree&apos;s ID when tagging it in the field.
          </Text>
...
const styles = StyleSheet.create({
  ...
  subtitle: { fontSize: 12, color: Colors.textMuted, marginTop: 2, marginBottom: 16 },
  qrWrap: {
    padding: 16,
    backgroundColor: "#FFFFFF",
    borderRadius: 16,
